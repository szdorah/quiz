...
export async function createQuiz({ title, description = "", questions = [] }) {
  const normalized = questions.map((q) => {
    ...
    const correctIndexes = options.map((option, index) => option?.isCorrect ? index : null).filter(index => index !== null);
    return {
...
export async function submitAnswer({ playerId, questionId, selectedIndex }) {
  try {
    const rows = await rpc("api_submit_answer", {
...
export async function nextQuestion(gameId) {
  const rows = await rpc("api_next_question", { p_game_id: gameId });
  const row = Array.isArray(rows) ? rows[0] : rows;
...
export async function leaderboard(gameId) {
  return (await rpc("api_leaderboard", { p_game_id: gameId })) || [];
}

export async function questionProgress(gameId, questionId) {
  const rows = await rpc("api_question_progress", { p_game_id: gameId, p_question_id: questionId });
